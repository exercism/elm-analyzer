module Exercise.ZebraPuzzleTest exposing (tests)

import Comment exposing (Comment, CommentType(..))
import Dict
import Exercise.ZebraPuzzle as ZebraPuzzle
...
rules : List Rule
rules =
    ZebraPuzzle.ruleConfig |> .rules |> List.map RuleConfig.analyzerRuleToRule
...
drinksWater : Maybe Nationality
drinksWater = Just Norwegian

ownsZebra : Maybe Nationality
ownsZebra = Just Japanese
"""
                |> Review.Test.run (ZebraPuzzle.hardcodingDrinksWater comment)
                |> Review.Test.expectErrors
                    [ TestHelper.createExpectedErrorUnder
                        comment
                        "drinksWater"
                        |> Review.Test.atExactly { start = { row = 10, column = 1 }, end = { row = 10, column = 12 } }
                    ]


hardcodingOwnsZebra : Test
hardcodingOwnsZebra =
    let
        comment =
            Comment "elm.zebra-puzzle.do_not_hardcode_solution" Essential Dict.empty
...
ownsZebra = Just Japanese
"""
                |> Review.Test.run (ZebraPuzzle.hardcodingOwnsZebra comment)
                |> Review.Test.expectErrors
                    [ TestHelper.createExpectedErrorUnder
                        comment
                        "ownsZebra"
                        |> Review.Test.atExactly { start = { row = 13, column = 1 }, end = { row = 13, column = 10 } }
                    ]
